...
| `src/adapters/openclaw/catalog.ts` | OpenClaw catalog discovery + projection. Enforces OAuth-only auth (B4), scopes the in-memory catalog cache per `(agentDir, workspaceDir, profileId)` (B3), allocates the S4 dispatch sequence before network I/O, and prevents split-brain or cross-profile catalog reuse on failure. Reuses core `listModels` + `isSafeModelId` + `isSafeEffortString` + `supportsThinking`. |
